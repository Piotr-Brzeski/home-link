...
void interrupt_function() {
  static uint32_t raw_state_history = 0;
  ...
  bool changed = false;
  if(raw_state) {
    if((raw_state_history & 0xFF0000FF) == 0xFF000000) {
      raw_state = false;
      raw_state_history = 0x00000000;
      changed = true;
    }
  }
  else {
    if((raw_state_history & 0xFF0000FF) == 0x000000FF) {
      raw_state = true;
      raw_state_history = 0xFFFFFFFF;
      changed = true;
    }
  }
  if(changed) {
